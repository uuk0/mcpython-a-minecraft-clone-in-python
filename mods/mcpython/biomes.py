...
class Biome:
    ...
    def GenerateBiome(self, model):
        for p in self.poslist:
            if p not in self.generated:
                ...
                model.add_block(
                    (x, 0, z), "minecraft:bedrock", save=False, immediate=False
                )
                for y in range(1, 5):
                    if round(Random.randint(self.seed, (x, y, z)) * 4) == 1:
                        model.add_block(
                            (x, y, z), "minecraft:bedrock", save=False, immediate=False
                        )
                    else:
                        model.add_block(
                            (x, y, z),
                ...
                for y in range(5, high):
                    if not self.hasOre(x, y, z, high):
                        ...
                        model.add_block(
                            (x, y, z), self.getOre(x, y, z), save=False, immediate=False
                        )
    ...
    def getHight(self, model, x, z):
        ...
        if y + hd >= self.getMinHigh() and y + hd <= self.getMaxHigh():
            y += hd
        elif hd > 0 and y + hd > self.getMaxHigh():
            y -= hd
        elif hd < 0 and y + hd < self.getMinHigh():
            y -= hd
        elif y > self.getMaxHigh():
            y = self.getMaxHigh()
        elif y < self.getMinHigh():
            y = self.getMinHigh()
        else:
            pass
        i = 0
...
class ColdTaiga(
    Biome
):  # missing: snow layers, snowing, grass, farn, flowers, mushrooms, wolfs, kaninchen
    def getName(self):
    ...
    def getHighDiffrents(self, x, z):
        return (
            round(Random.randint(self.seed, [x, z, 123]) * 4 - 2)
            if round(Random.randint(self.seed, [x, z, 0]) * 3) == 1
            else 0
        )
    ...
    def hasOre(self, x, y, z, high):
        r = (
            round(Random.randint(self.seed, [x, y, z, high]) * 15) == 1
            and high / 1.5 > y
        )
        return r
    ...
    def getMaterial(self, x, y, z, high):
        return (
            "minecraft:stone"
            if high / 1.5 > y
            else ("minecraft:dirt" if y != high - 1 else "minecraft:grass")
        )

    def getStructur(self, pos, high):
        r = round(Random.randint(self.seed, [pos, high]) * 30)
        if r == 1:
            return structures.handler.structures[self.getStructures()[0]]
        return None
...
class ColdTaiga_M(ColdTaiga):
    def getHighDiffrents(self, x, z):
        return (
            round(Random.randint(self.seed, [x, z, 123]) * 8 - 4)
            if round(Random.randint(self.seed, [x, z, 0]) * 3) == 1
            else 0
        )
...
class ColdTaigaHills(ColdTaiga):
    def getHighDiffrents(self, x, z):
        return (
            round(Random.randint(self.seed, [x, z, 123]) * 6 - 3)
            if round(Random.randint(self.seed, [x, z, 0]) * 3) == 1
            else 0
        )
...
class ColdTundraIcicle(ColdTundra):
    def getStructures(self):
        return [
            "minecraft:structure:spruce_tree",
            "minecraft:structure:icicle",
            "minecraft:structure:oak_tree",
        ]
    ...
    def getStructur(self, pos, high):
        ...
        if r == 1:
            ...
            return structures.handler.structures[self.getStructures()[1]]
        elif r == 5:
            return structures.handler.structures[self.getStructures()[2]]
        return None
