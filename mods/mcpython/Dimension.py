...
class Dimension:
    def __init__(self):
        self.id = IDGenerator.handler.getDimensionId() if not self.getId() else self.getId()
        self.chunks = {}
        self.biomedata = {}
        self.highdata = {}
        self.biomeinst = {}
    ...
    def generateChunk(self, chunk):
        ...
        for x in range(chunk[0]*16, chunk[0]*16+16):
            for z in range(chunk[1]*16, chunk[1]*16+16):
                if not (x, z) in self.biomedata:
                    raise ValueError("getted an unprepared area-biome-data", x, z)
                self.biomeinst[self.biomedata[(x, z)]].poslist.append((x, z))
                if not self.biomeinst[self.biomedata[(x, z)]] in bios: bios.append(self.biomeinst[self.biomedata[(x, z)]])
        for biome in bios:
        ...
        if random.randint(1, 5) == 1 and config.CONFIGS["GENERATE_PERLIN"]:
            ...
            for _ in range(random.randint(1, 5)):
                x, z = random.randint(bpos[0], bpos[0]+15), random.randint(bpos[1], bpos[1]+15)
            ...
            for xl in data:
                for zl in xl:
                    ...
                    if not (cx, cz) in ch: ch.append((cx, cz))
                    i += 1
                    if time.time() - dt > 1:
                        dt = time.time()
                            ...
                            if (x, z) in self.perlin_positions:
                                for y, v in enumerate(self.perlin_positions[(x, z)]):
                                    if v >= 0 and (x, y, z) in G.model.world:
                                        G.model.remove_block((x, y, z), immediate=False, save=False)
                                    if time.time() - dt > 1:
                                        dt = time.time()
                                        print(str(round(i / len(self.perlin_positions[chunk]) * 100)) + "%")
        ...
        for x in range(-size, size+1):
            for z in range(-size, size+1):
                ...
                for p in [(x+1, z), (x-1, z), (x, z+1), (x, z-1)]:
                    if p in self.biomedata: surrounding.append(self.biomedata[p])
                ...
                if type(biome) != int: biome = self.getBiomes().index(biome)
                if not biome in self.biomeinst:
                    self.biomeinst[biome] = self.getBiomes()[biome]([], G.seed)
                self.biomeinst[biome].getHight(G.model, x, z)
                self.biomedata[(x, z)] = biome
                index += 1
                if time.time() - last > 1:
    ...
    def load(self):
        G.model.world = self.worlddata

    def unload(self):
        self.worlddata = G.model.world
...
class Overworld(Dimension):
    def getBiomes(self):
        return [biomes.ColdTaiga, biomes.ColdTaiga_M, biomes.ColdTaigaHills, biomes.ColdTaigaHillsBig,
                biomes.ColdTundra, biomes.ColdTundraIcicle]
