...
class furnes(Block):
    ...
    def crafting_update(self, slot, item, amount):
        if slot == self.inv.slots[0] and slot.item:
            for e in crafting.craftinghandler.recipis[crafting.Grid.crafting_smelting]:
                if e.input[0] == self.inv.slots[0].item.getName() or e.input[0] in self.inv.slots[0].item.getOreDictNames():
                    if e.inputamount[0] >= self.inv.slots[0].amount and (
                        not self.inv.slots[2].item or self.inv.slots[2].item.getName() == e.output[0]):
                        if e != self.crafting_recipi:
                            self.crafting_recipi = e
                            self.burnstarttime = time.time()
                            self.storedfuel = 0
                        if not self.running: TickHandler.handler.run(self.tickUpdate, 1)
                        return
            self.crafting_recipi = None
            self.burnstarttime = None
        elif slot == self.inv.slots[1] and slot.item and self.crafting_recipi and not self.running:
    ...
    def tickUpdate(self):
        ...
        if self.storedfuel >= self.crafting_recipi.kwargs["fuel"]:
            if not self.inv.slots[2].item:
                ...
                print("[ERROR] during checking furnes recipi: furnes recipi is set but output is incorrect")
                self.crafting_recipi = None
                self.running = False
                return
            ...
            if self.inv.slots[1].item:
                if self.inv.slots[1].item.getFuelAmount() > 0 and self.fuelamount + self.inv.slots[1].item.getFuelAmount() <= self.maxstoreablefuel:
            ...
            if time.time() >= self.fueltime + 0.5:
                ...
                fuel = 5 if not "fuelpertime" in self.crafting_recipi.kwargs else self.crafting_recipi.kwargs["fuelpertime"]
    ...
    def getTex(self):
        if self.fueltime > 0:
            return total_tex_coords((13, 14), (13, 14), (14, 15), (13, 15), (13, 15), (13, 15))
        else:
            return total_tex_coords((13, 14), (13, 14), (14, 14), (13, 15), (13, 15), (13, 15))
