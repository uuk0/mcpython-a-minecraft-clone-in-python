...
class TileStateWorldModel(TileState):
    ...
    def activate(self):
        self.events.append(
            EventHandler.eventhandler.on_event("on_draw_3D", self.draw3d)
        )
...
class TileStateOptionBackground(TileState):
    def __init__(self):
        ...
        self.picture = pyglet.sprite.Sprite(
            texturGroups.handler.groups["./assets/textures/gui/options_background.png"]
        )
    ...
    def activate(self):
        self.events.append(
            EventHandler.eventhandler.on_event("on_draw_2D", self.draw2d)
        )
...
class TileStateMouseExternal(TileState):
    def __init__(self):
        TileState.__init__(self)
        self.picture = pyglet.sprite.Sprite(
            texturGroups.handler.groups["./assets/textures/gui/mousetrigger.png"]
        )
    ...
    def activate(self):
        ...
        self.events.append(
            EventHandler.eventhandler.on_event("on_mouse_press", self.key_press)
        )
...
class Game(State):
    def __init__(self):
        ...
        self.resize(
            ...
            0,
            0,
            config.CONFIGS["DEFAULT_WINDOW_SIZE"][0],
            config.CONFIGS["DEFAULT_WINDOW_SIZE"][1],
        )
    ...
    def activate(self):
        ...
        self.events.append(
            EventHandler.eventhandler.on_event("on_key_press", self.key_press)
        )
        self.events.append(
            EventHandler.eventhandler.on_event("on_key_release", self.key_releas)
        )
        self.events.append(
            EventHandler.eventhandler.on_event("on_mouse_motion", self.mouse_motion)
        )
        self.events.append(
            EventHandler.eventhandler.on_event("on_mouse_press", self.mouse_press)
        )
    ...
    def draw2d(self, *args):
        ...
        if (round(x), round(z)) in G.player.dimension.biome_data:
            text += (
                "biom: "
                + G.player.dimension.biome_instances[
                    G.player.dimension.biome_data[(round(x), round(z))]
                ].getName()
            )
        ...
        if block and config.CONFIGS["SHOW_BLOCK_LOOKING_AT"]:
            self.label2.text = (
                "You are looking at "
                + G.model.world[block].getVisualName()
        ...
        if symbol == config.CONFIGS["init"]["KEYBINDS"]["forward:key"]:
            ...
            if G.player.mode == 1 and len(G.inventoryhandler.shown) <= 1:
                ...
                if G.player.inventory.moving_slot and G.player.inventory.moving_start:
                    G.player.inventory.moving_slot.setPos(
                        *G.player.inventory.moving_start
                    )
            ...
            symbol == config.CONFIGS["init"]["KEYBINDS"]["togleflying:key"]
            and G.player.gamemode == 1
        ):
            ...
            symbol == config.CONFIGS["init"]["KEYBINDS"]["movecamera:key"][0]
            and config.CONFIGS["ALLOW_CAMERA_MOVING_WITH_ARROWS"]
        ):
            G.window.turning_strafe[1] = 1
        elif (
            symbol == config.CONFIGS["init"]["KEYBINDS"]["movecamera:key"][1]
            and config.CONFIGS["ALLOW_CAMERA_MOVING_WITH_ARROWS"]
        ):
            G.window.turning_strafe[1] = -1
        elif (
            symbol == config.CONFIGS["init"]["KEYBINDS"]["movecamera:key"][3]
            and config.CONFIGS["ALLOW_CAMERA_MOVING_WITH_ARROWS"]
        ):
            G.window.turning_strafe[0] = 1
        elif (
            symbol == config.CONFIGS["init"]["KEYBINDS"]["movecamera:key"][2]
            and config.CONFIGS["ALLOW_CAMERA_MOVING_WITH_ARROWS"]
        ):
            G.window.turning_strafe[0] = -1
    ...
    def mouse_press(self, eventname, x, y, button, modifiers):
        ...
        if G.window.exclusive and G.player.gamemode != 3 and G.player.gamemode != 2:
            ...
            if (button == mouse.RIGHT) or (
                ...
                if (
                    block
                    and G.model.world[block].hasInventory()
                    and (
                        not G.player.inventory.hotbar.slots[G.window.hotbarelement].item
                        or (
                            G.player.inventory.hotbar.slots[G.window.hotbarelement].item
                            and not OreDict.ORE_DROP
                            in G.player.inventory.hotbar.slots[
                                G.hotbarelement
                            ].item.getOreDictNames()
                    ...
                    handler.activate(
                        invhandler.inventoryinst[
                            G.model.world[block].getInventoryID()
                        ].eventname
                    )
                    print(
                        G.model.world[block].getInventorys()[0].eventname,
                        invhandler.inventoryinst[G.model.world[block].getInventoryID()],
                    )
                    print("showing inventory", G.model.world[block].getInventorys())
                elif (
                    G.player.inventory.hotbar.slots[G.window.hotbarelement].item
                    and OreDict.ORE_DROP
                    in G.player.inventory.hotbar.slots[
                        G.window.hotbarelement
                    ].item.getOreDictNames()
                    and block
                    and G.model.world[block].getName() == "minecraft:stone"
                ):
                    G.model.add_block(
                        block,
                        "minecraft:"
                        + G.player.inventory.hotbar.slots[
                            G.window.hotbarelement
                        ].item.getOreMaterial()
                        + "_ore",
                        hitblock=previous,
                    ...
                    if (
                        slot.item
                        and block in G.model.world
                        and slot.item.canInteractWith(G.model.world[block])
                    ):
                        slot.item.interactWith(G.model.world[block])
                    elif (
                        slot.item.hasBlock()
                        and block in G.model.world
                        and not G.model.world[block].hasInventory()
                        and not slot.item.canInteractWith(G.model.world[block])
                    ):
                        if G.model.add_block(
                            previous, slot.item.getBlockName(), hitblock=previous
                        ):
                ...
                if block:
                    if (
                        (
                            not G.player.inventory.hotbar.slots[
                                G.window.hotbarelement
                            ].item
                        )
                        or G.player.inventory.hotbar.slots[
                            G.window.hotbarelement
                        ].item.canDestroyBlock(block, button, modifiers)
                        and G.model.world[block].isBreakAble()
                        ...
                        G.player.inventory.hotbar.slots[
                            ...
                            G.window, block, previous, button, modifiers
                        )
...
class ChatState(State):
    ...
    def activate(self):
        self.events.append(
            EventHandler.eventhandler.on_event("on_draw_2D", self.draw_2d)
        )
        self.events.append(
            EventHandler.eventhandler.on_event("on_key_press", self.key_press)
        )
...
class DemoInfo(State):
    def __init__(self):
        State.__init__(self)
        self.demoinfosprite = pyglet.sprite.Sprite(
            texturGroups.handler.groups["./assets/textures/gui/demomsg.png"]
        )
    ...
    def activate(self):
        ...
        self.events.append(
            EventHandler.eventhandler.on_event(
                "on_demo_info_purchase_now_clicked",
                self.on_demo_info_purchase_now_clicked,
            )
        )
        self.events.append(
            EventHandler.eventhandler.on_event(
                "on_demo_info_continue_plaing_clicked",
                self.on_demo_info_continue_plaing_clicked,
            )
        )
        self.events.append(
            EventHandler.eventhandler.on_event("on_key_press", self.key_press)
...
class StartMenü(State):
    def __init__(self):
        ...
        self.game_start_worldselect = pyglet.sprite.Sprite(
            texturGroups.handler.groups[
                "./assets/textures/gui/startgui_singelplayer.png"
            ]
        )
    ...
    def getDependencies(self):
        return [
            "minecraft:tilestate:options:background",
            "minecraft:tilestate:mouseexternal",
        ]
    ...
    def mouse_press(self, eventname, x, y, button, modifiers):
        if (
            x > self.game_start_worldselect.x
            and x
            < self.game_start_worldselect.x + self.game_start_worldselect.image.width
            and y > self.game_start_worldselect.y
            and y
            < self.game_start_worldselect.y + self.game_start_worldselect.image.height
        ):
            G.window.set_menu("minecraft:world_select")
        elif (
            x > self.game_start_quit_sprite.x
            and x
            < self.game_start_quit_sprite.x + self.game_start_quit_sprite.image.width
            and y > self.game_start_quit_sprite.y
            and y
            < self.game_start_quit_sprite.y + self.game_start_quit_sprite.image.height
        ):
            G.window.close()
        elif modifiers & key.MOD_SHIFT:
            print(x, y)
...
class WorldSelect(State):
    def __init__(self):
        ...
        self.world_select_creat_new_sprite = pyglet.sprite.Sprite(
            texturGroups.handler.groups[
                "./assets/textures/gui/worldselect_creat_new.png"
            ]
        )
    ...
    def mouse_press(self, eventname, x, y, button, modifiers):
        if (
            x > self.world_select_creat_new_sprite.x
            and x
            < self.world_select_creat_new_sprite.x
            + self.world_select_creat_new_sprite.image.width
            and y > self.world_select_creat_new_sprite.y
            and y
            < self.world_select_creat_new_sprite.y
            + self.world_select_creat_new_sprite.image.height
        ):
            G.window.set_menu("minecraft:loading")
            # TickHandler.handler.run(self.generate, 2)
            self.generate()
        elif (
            x > self.world_select_canceled_sprite.x
            and x < self.world_select_canceled_sprite.image.width
            and y > self.world_select_canceled_sprite.y
            and y
            < self.world_select_canceled_sprite.y
            + self.world_select_canceled_sprite.image.height
        ):
