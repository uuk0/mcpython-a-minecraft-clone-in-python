...
class Model(object):
    ...
    def hit_test(self, position, vector, max_distance=8):
        ...
        for _ in xrange(max_distance * m):
            key = normalize((x, y, z))
            if key != previous and key in self.world and self.world[key].hasHitbox(self.window.player.inventory.hotbar.slots[self.window.hotbarelement].item):
                return key, previous
            previous = key
    ...
    def add_block(self, position, block, immediate=True, inst=False, creat=False, hitblock=None, save=True):
        ...
        if position[1] > 256 or position[1] < 0: return False
        if not inst:
            oblock = block
        ...
        if self.window.worldname and save:
            WorldSaver.saveToChunk(sectorize(position), self.window.worldname, [0, position, self.world[position].getName()])
        return True
    ...
    def remove_block(self, position, immediate=True, save=True, call=True):
        ...
        if call: self.world[position].on_destroy()
        if immediate:
        ...
        if immediate:
            ...
            self.updateNexts(position)
        if self.window.worldname and save:
            WorldSaver.saveToChunk(sectorize(position), self.window.worldname, [2, position])
    ...
    def updateTex(self, position):
        ...
        self.hide_block(position)
        if state: self.show_block(position)
