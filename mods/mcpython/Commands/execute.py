...
class execute(Command.Command):
        ...
        while i <= len(s):
            if s[i] == "as":
                ...
                position = Command.getPosition(
                    s[i + 1], s[i + 2], s[i + 3], position, entity
                )
                i += 2
            elif s[i] == "if":
                if s[i + 1] == "block":
                    pos = Command.getPosition(
                        s[i + 2], s[i + 3], s[i + 4], position, entity
                    )
                    blockname = s[i + 5]
                    if (
                        not pos in G.model.world
                        or not G.model.world[pos].getName() == blockname
                    ):
                        return
                    s += 6
                elif s[i + 1] == "entity":
    ...
    def getHelp():
        return [
            "/execute {[as <entity>], [at <entity>], [if/unless [block <position> <block>, blocks <position1> <position2> <position>, entity <entity> #missing:score]], [in <dimenson-id], [run <undercommand>], }"
        ]
