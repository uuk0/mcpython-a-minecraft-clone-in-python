...
class WriteStr(Command.Command):
        ...
        if len(s) == 1:
            ...
            for e in Command.getSelector(s[1], position, entity):
                if not (hasattr(e, "writeToChat") and callable(e.writeToChat)):
                    return ["at "+s[1]+" : invalid selector: excpected only players, gotted ", e,
                            "    ^"]
    ...
    @staticmethod
    def getHelp():
        return ["/msg <players> <msg>: prints the msg by every entity",
