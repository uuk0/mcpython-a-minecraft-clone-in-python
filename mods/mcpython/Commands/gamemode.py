...
class gamemode(Command.Command):
    ...
    def getSyntaxError(line, entity, position, chat):
        ...
        if len(s) == 1:
            return [
                "at " + line + " : missing argument <gamemode>",
                "   " + " " * len(line) + "^",
            ]
        if s[1] not in [
            "survival",
        ...
        if len(s) == 3 and Command.getSelector(s[2], position, entity) == []:
            return [
                "at " + s[2] + " : invalid argument: excpected entity OR entityselector"
            ]
        for e in (
            Command.getSelector(s[2], position, entity) if len(s) == 3 else [entity]
        ):
    ...
    def parse(line, entity, position, chat):
        ...
        if gamemode in ["survival", "creative", "hardcore", "spectator"]:
            gamemode = ["0", "1", "2", "3"][
                ["survival", "creative", "hardcore", "spectator"].index(gamemode)
            ]
        if len(s) == 3:
